    ...
    var encjson = (function() {
         ...
         * @param {Number} iterations Number of iterations of the algorithm (default: 1000 iterations)
         * @return
         */
        module.setpassword = function(pwd, iterations) {
            ...
            password = CryptoJS.PBKDF2(pwd, CryptoJS.lib.WordArray.random(
                128 / 8), {
                keySize: 512 / 32,
                iterations: iterations
            }).toString();
            passwordProp = CryptoJS.PBKDF2(pwd, CryptoJS.lib.WordArray
                .random(128 / 8), {
                    keySize: 512 / 32,
                    iterations: iterations
                }).toString();
         ...
         * @method setrandompassword
         * @return
         */
        module.setrandompassword = function() {
                var N = Math.floor(Math.random() * 8) + 8;
                var pwd = new Array(N + 1).join((Math.random().toString(
                    36) + '00000000000000000').slice(2, 18)).slice(
                    0, N);
                module.setpassword(pwd);
            }
            /**
             * Sets the password without using any password-based key derivation function.
             * @method setrawpassword
        ...
        /**
         * Converts a string to its real type
         * @method parseString
         * @param {String} value The string you want to convert
         * @return The converted string object of its real type.
         */
        function parseString(value) {
            ...
            /**
             * Encrypts recursively a JSON
             * @method recursiveEncryptJSON
             * @param {Object} obj The object you want to encrypt
             * @return destObj
             */
        ...
        function recursiveEncryptJSON(obj) {
                ...
                for (var prop in obj) {
                    if (obj.hasOwnProperty(prop)) {
                        var strName = 'K' + CryptoJS.Rabbit.encrypt(
                            prop, passwordProp).toString();
                        if (Object.prototype.toString.call(obj[prop]) ===
                            ...
                            for (var i = 0; i < aux.length; i++) {
                                if (aux[i] instanceof Object) {
                                    destObj[strName][i] =
                                        recursiveEncryptJSON(aux[i]);
                                } else {
                                    destObj[strName][i] = CryptoJS.Rabbit
                                        .encrypt('' + aux[i], password)
                                        .toString();
                                }
                            }
                        } else if (obj[prop] instanceof Object) {
                            destObj[strName] = recursiveEncryptJSON(obj[
                                prop]);
                        } else {
                            destObj[strName] = {};
                            destObj[strName] = CryptoJS.Rabbit.encrypt(
                                '' + obj[prop], password).toString();
                        }
                    }
                }
                return destObj;
            ...
            /**
             * Decrypts recursively a JSON
             * @method recursiveDecryptJSON
             * @param {Object} obj The object you want to decrypt
             * @return destObj
             */
        ...
        function recursiveDecryptJSON(obj) {
                ...
                for (var prop in obj) {
                    if (obj.hasOwnProperty(prop)) {
                        var strName = CryptoJS.Rabbit.decrypt(prop.substring(
                            1), passwordProp).toString(CryptoJS.enc
                            .Utf8);
                        if (Object.prototype.toString.call(obj[prop]) ===
                            '[object Array]') {
                            destObj[strName] = [];
                            ...
                            for (var i = 0; i < aux.length; i++) {
                                if (aux[i] instanceof Object) {
                                    destObj[strName][i] =
                                        recursiveDecryptJSON(aux[i]);
                                } else {
                                    destObj[strName][i] = parseString(
                                        CryptoJS.Rabbit.decrypt(aux[
                                            i], password).toString(
                                            CryptoJS.enc.Utf8));
                                }
                            }
                        } else if (obj[prop] instanceof Object) {
                            destObj[strName] = recursiveDecryptJSON(obj[
                                prop]);
                        } else {
                            destObj[strName] = {};
                            destObj[strName] = parseString(CryptoJS.Rabbit
                                .decrypt(obj[prop], password).toString(
                                    CryptoJS.enc.Utf8));
                        }
                    }
                }
             ...
             * @method encryptjson
             * @param {Object} obj The object you want to encrypt
             * @return The encrypted JSON preserving its structure
             */
        module.encryptjson = function(obj) {
            if (password === undefined) {
                throw new Error(
                    "A password needs to be set before calling this function"
                )
         ...
         * @method encryptproperty
         * @param {String} prop The property to be encrypted
         * @return The encrypted property string
         */
        module.encryptproperty = function(prop) {
            ...
            return 'K' + CryptoJS.Rabbit.encrypt(prop, passwordProp)
                .toString();
        };
        /**
         * Decrypts a JSON property. A password needs to be set before calling this function
         * @method decryptproperty
         * @param {String} prop The property to be encrypted
         * @return The decrypted property string
         */
        module.decryptproperty = function(prop) {
            ...
            return CryptoJS.Rabbit.decrypt(prop.substring(1),
                passwordProp).toString(CryptoJS.enc.Utf8);
        };
        /**
         * Encrypt a JSON value. A password needs to be set before calling this function.
         * @method encryptvalue
         * @param {Object} val The value to be encrypted. It can be of one the following types:
         * number, string, boolean or null
         * @return The encrypted value as a string encoded in hexadecimal
         */
         ...
         * @return The decrypted value object of its real type
         */
        module.decryptvalue = function(val) {
            ...
            return parseString(CryptoJS.Rabbit.decrypt(val,
